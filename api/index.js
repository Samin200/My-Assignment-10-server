...
const { getDB, ObjectId } = require("../lib/db");

const app = express();
app.use(express.json());
app.use(cors());

// Routes (mimic existing routes; DB access is via getDB())
app.get("/", async (req, res) => {
  res.send("My assignment 10 server is running");
});

app.get("/movies", async (req, res) => {
  const { moviesCollection } = await getDB();
...
app.patch("/movies/:id", async (req, res) => {
  try {
    ...
    const allowedFields = [
      "title", "genre", "releaseYear", "director", "cast", "rating",
      "duration", "plotSummary", "posterUrl", "language", "country", "addedBy",
    ];
